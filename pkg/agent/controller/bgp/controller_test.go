		...
		},
		{
			...
			expectedState: generateBGPPolicyState(bgpPolicyName1,
				...
				func(state *bgpPolicyState) {
					state.confederationConf = &confederationConfig{100, sets.New[uint32](uint32(65001))}
				}),
			expectedCalls: func(mockBGPServer *bgptest.MockInterfaceMockRecorder) {
				mockBGPServer.Start(gomock.Any())
				mockBGPServer.AddPeer(gomock.Any(), ipv4Peer1Config)
...
func TestBGPPolicyUpdate(t *testing.T) {
	...
	patchBGPPolicyStateConfederationFn2 := func(state *bgpPolicyState) {
		state.confederationConf = &confederationConfig{100, sets.New[uint32](uint32(65002))}
	}
	patchBGPPolicyStateConfederationFn3 := func(state *bgpPolicyState) {
		state.confederationConf = &confederationConfig{101, sets.New[uint32](uint32(65001))}
	}
...
func TestSyncBGPPolicyFailures(t *testing.T) {
	...
	// The local ASN of BGPPolicy policy2 is not the same as BGPPolicy policy1, and the current BGP server should be stopped.
...
func deepCopyBGPPolicyState(in *bgpPolicyState) *bgpPolicyState {
	...
	if in.confederationConf != nil {
		confederationConf = &confederationConfig{
			identifier: in.confederationConf.identifier,
			peers:      in.confederationConf.peers.Clone(),
		}
	}
	...
	return &bgpPolicyState{
		...
		confederationConf: confederationConf,
		routes:            routes,
		peerConfigs:       peerConfigMap,
	}
}
...
func checkBGPPolicyState(t *testing.T, expected, got *bgpPolicyState) {
	...
	if expected != nil {
		...
		assert.Equal(t, expected.confederationConf, got.confederationConf)
	}
}
...
func TestGetBGPPolicyInfo(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			actualBgpPolicyName, actualRouterID, actualASN, actualListenPort := c.GetBGPPolicyInfo()
