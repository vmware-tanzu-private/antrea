...
type featureNetworkPolicy struct {
	...
	// to antrea-agent using the packetIn mechanism, the other is to force the packet to continue forwarding the
...
func (f *featureNetworkPolicy) l7NPTrafficControlFlows() []binding.Flow {
	...
	flows := []binding.Flow{
		...
		// the packets to stageConntrackState directly. Note that, for the packets which are originally to be output to a
